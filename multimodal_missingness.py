...
class Config:
    ...
    # Task parameters generated by data_preprocessing.py
...
class EnhancedMMNARModelWithSSL(nn.Module):
    ...
    def forward(self, x_struct, x_img, x_text, x_rad, missing_flags):
        ...
        #    Match dataset missing_pattern encoding: S=8, I=4, T=2, R=1.
        pattern_ids = torch.sum(
            missing_flags * torch.tensor([8, 4, 2, 1], device=missing_flags.device),
            dim=1
        ).long()
...
class MultiModalDataset(Dataset):
    ...
    def __init__(self, struct_path, img_path, text_path, normalize=True, train_mode=True):
        ...
        exclude_cols = ['subject_id', config.LABEL_READM, config.LABEL_ICU, config.LABEL_MORTALITY]
        leaking_features = ['idx_discharge_location', 'idx_severity_mode', 'idx_hospital_expire_flag']
    ...
    def print_statistics(self):
        ...
        print(f"90-day ICU readmission rate: {self.df[config.LABEL_ICU].mean()*100:.2f}%")
        print(f"180-day death during readmission rate: {self.df[config.LABEL_MORTALITY].mean()*100:.2f}%")
...
def evaluate_with_tta(model, loader, device, tta_augmenter=None):
    ...
    # Average losses over the samples actually yielded by the loader/sampler.
    n_samples = len(all_readm_targets)
...
def train_and_evaluate_enhanced_complete(train_loader, val_loader, test_loader=None):
    ...
    # Stepped once per epoch below, so configure OneCycleLR with one step per epoch.
    ...
    if test_loader is not None:
        ...
        print("-"*50)
        print(f"{'Readmission 30d':<15} {test_metrics['readm_auc']:<10.4f} {test_metrics['readm_apr']:<10.4f} {test_metrics['readm_brier']:<10.4f}")
        print(f"{'ICU 90d':<15} {test_metrics['icu_auc']:<10.4f} {test_metrics['icu_apr']:<10.4f} {test_metrics['icu_brier']:<10.4f}")
        print(f"{'Mortality 180d':<15} {test_metrics['mortality_auc']:<10.4f} {test_metrics['mortality_apr']:<10.4f} {test_metrics['mortality_brier']:<10.4f}")
        print(f"{'='*80}")
...
def main():
    ...
    for split_name, split_idx in [("Train", train_idx), ("Val", val_idx), ("Test", test_idx)]:
        ...
        print(f"{split_name}: Mortality180d={mortality_rate:.3f}, Readmission30d={readm_rate:.3f}, "
              f"ICU90d={icu_rate:.3f}, Complete={complete_rate:.3f}")
    ...
    if test_predictions:
        ...
        tasks = [('Readmission 30d', 'readm'), ('ICU 90d', 'icu'), ('Mortality 180d', 'mortality')]
